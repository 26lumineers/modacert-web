    ...
    <AppFrame currentStep={4}>
      <section className="px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionTitle
            eyebrow="Step 2"
            title="Upload the pictures of your bag one by one."
            description="The controls are static for now. The page shows the required checklist for a future upload integration."
          />
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {uploadRequirements.map((item) => (
              <article key={item.title} className="rounded-[1.5rem] bg-white p-4 shadow-card">
                <div className="relative aspect-[4/3] overflow-hidden rounded-[1.15rem] bg-product">
                  <Image src={item.image} alt="" fill sizes="(min-width: 1024px) 28vw, 86vw" className="object-cover" />
            ...
            <article className="grid min-h-[280px] place-items-center rounded-[1.5rem] border-2 border-dashed border-mc-orange/42 bg-white/68 p-6 text-center">
              <div>
                ...
                <h2 className="mt-4 text-lg font-bold">Additional photo</h2>
          ...
          <div className="mt-10 text-center">
            <ButtonLink href="/" tone="dark">Submit static checklist</ButtonLink>
          </div>
        </div>
      </section>
    </AppFrame>
